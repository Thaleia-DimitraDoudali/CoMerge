...
GridPoint * generate_energy_grid( long n_isotopes, long n_gridpoints,
	...
	//                                               * sizeof( GridPoint ) );
	GridPoint * energy_grid = (GridPoint *) numa_alloc_onnode( n_unionized_grid_points
	                                               * sizeof( GridPoint ), 1);
	...
	//int * full = (int *) malloc( n_isotopes * n_unionized_grid_points
	  //                           * sizeof(int) );
	int * full = (int *) numa_alloc_onnode( n_isotopes * n_unionized_grid_points
	                            * sizeof(int), 1);
    printf("Energy Grid size = %lf\n", (n_unionized_grid_points*sizeof(GridPoint) + n_isotopes * n_unionized_grid_points*sizeof(int))/1048576.0);
	if( full == NULL )
	{
