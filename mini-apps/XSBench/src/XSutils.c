...
/*	NuclideGridPoint * full = (NuclideGridPoint *) malloc( m * n *
	...
	NuclideGridPoint ** M = (NuclideGridPoint **) malloc( m *
	                          sizeof(NuclideGridPoint *) );*/
	NuclideGridPoint * full = (NuclideGridPoint *) numa_alloc_onnode( m * n *
	                          sizeof( NuclideGridPoint ), 0);
	NuclideGridPoint ** M = (NuclideGridPoint **) numa_alloc_onnode( m *
	                          sizeof(NuclideGridPoint *), 0);
    printf("NGP size = %lf\n", (m*n*sizeof(NuclideGridPoint) + m*sizeof(NuclideGridPoint *)) / 1048576.0);
	for( i = 0, j=0; i < m*n; i++ )
		if( i % n == 0 )
			M[j++] = &full[i];
